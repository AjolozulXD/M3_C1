...
@app.route("/")
def hola():
    return '<h1>Croissant</h1> <a href = /datitos> Haz click aquí para ver datos de la dependencia tecnológica</a> <a href = /funcion_extra> Haz click aquí para lanzar una moneda</a>'

@app.route("/datitos")
def info():
    return f'<p>{random.choice(datos)}</p>'

@app.route("/funcion_extra")
def moneda():
    return f'<p>{random.choice(resultado)}</p>'
